# The normal script, settings are done through user input
...
def InputSanityCheck(inp): # should've been named InputPortSanityCheck
	while True:
		...
		try:
			...
			if p1 >= 1 and p1 <= 65535:
				return p1
			else:
				print("Not a valid port, please input a number 1-65535")

port = InputSanityCheck("What port would you like to listen to locally: ")

while True:
	addr = input("Would you like to listen on localhost or 0.0.0.0 [local] [0]: ")
	if addr == "local":
		laddr = "localhost"
		break
	elif addr == "0":
		laddr = "0.0.0.0"
		break
	elif match(IPRegex,addr):
		laddr = addr
		break
	else:
		print("Invalid input, please input local, 0, or an IP address to listen on")

while True:
	addr2 = input("What is the IP/Domain of the domain you want to proxy: ")
	if addr2 == "local":
		addr2 = "localhost"
		break
	elif match(IPRegex,addr2):
		break
	elif match(DomainRegex,addr2):
		print("WARNING: detected a domain, attempting to proxy to {0}".format(addr2))
		break
	else:
		print("Not a valid input. Input an IP address or a domain without a proto (domain.name)")

rport = InputSanityCheck("What port do you want to proxy: ")
